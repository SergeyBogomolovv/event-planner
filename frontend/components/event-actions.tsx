...
export function EventActions({ event, compact = false }: EventActionsProps) {
  ...
  const linkClass = cn(buttonVariants({ variant: 'outline', size }), 'gap-2', !compact && outlineClass)
    ...
    <div className='space-y-2'>
      <div className='flex flex-wrap gap-2'>
        ...
        {event.availableActions.includes('publish') ? (
          <Button variant='outline' size={size} className={outlineButtonClass} disabled={pendingAction === 'publish'} onClick={() => runAction('publish')}>
            <Megaphone className='h-4 w-4' />
            {pendingAction === 'publish' ? 'Публикация...' : actionLabels.publish}
          </Button>
        ) : null}
        {event.availableActions.includes('cancel') ? (
          <Button variant='outline' size={size} className={outlineButtonClass} disabled={pendingAction === 'cancel'} onClick={() => runAction('cancel')}>
            <XCircle className='h-4 w-4' />
            {pendingAction === 'cancel' ? 'Отмена...' : actionLabels.cancel}
          </Button>
        ) : null}
        {event.availableActions.includes('complete') ? (
          <Button variant='outline' size={size} className={outlineButtonClass} disabled={pendingAction === 'complete'} onClick={() => runAction('complete')}>
            <CheckCircle2 className='h-4 w-4' />
            {pendingAction === 'complete' ? 'Завершение...' : actionLabels.complete}
          </Button>
        ) : null}
        {event.availableActions.includes('delete') ? (
          <Button variant='destructive' size={size} className='gap-2' disabled={pendingAction === 'delete'} onClick={() => runAction('delete')}>
